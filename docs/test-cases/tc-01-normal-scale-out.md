...
- sessionCount=81이 Scale-out threshold 80을 초과한다.
- Control Plane이 Scale-out을 결정한다.
- ECS desiredCount가 1 → 2로 증가한다.
- EC2 capacity가 부족한 경우 ASG desired capacity가 1 → 2로 증가한다.
- 신규 EC2 instance가 ECS Cluster에 등록된다.
- 신규 Task가 RUNNING 상태에 도달한다.
- 기존 Task의 WebSocket session은 유지된다.
- 최종 상태는 `desired=2 / running=2 / pending=0`이다.
